...
# Heavily modified for Telefang use.
...
def convert_to_wav(filenames=[], options={}):
    """
    Converts a file containing 4-bit pcm data into a .wav file.
    """
    for filename in filenames:
        ...
        for value in samples:
            exp_value = min(0xFF, value << 4)
            packed_value = struct.pack('B', exp_value)
...
def convert_to_pcm(filenames=[], options={}):
    ...
    Samples in the .wav file are simply clamped to 4 bits, no dithering yet.
    ...
    """
    for filename in filenames:
        samples, average_sample, peak_sample = get_wav_samples(filename)
        ...
        for i in xrange(0, len(clamped_samples), 2):
            ...
            packed_value = 0
            for j in range(2):
                packed_value += clamped_samples[i + j] << ((1 - j) * 4)
...
def get_wav_samples(filename):
    ...
    return resampled_samples, average_sample, largest_sample
